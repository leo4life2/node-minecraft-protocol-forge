...
const { NESTED_JAR_RE, MAX_NESTED_DEPTH, readNestedManifest, nestedArtifactLabel } = require('../src/client/nestedJars')
...
describe('HF53 - nested JarJar login-channel corroboration', function () {
  ...
  it('N4 the manifest names the nested artifacts; the one rule accepts both spellings and nothing else', () => {
    ...
    assert.ok(!NESTED_JAR_RE.test('META-INF/jarjar/metadata.json') && !NESTED_JAR_RE.test('lib/c.jar') && !NESTED_JAR_RE.test('META-INF/jarjar/deep/d.jar'))
    assert.strictEqual(MAX_NESTED_DEPTH, 2)
    // the other derivers hold no private spelling of the rule any more
    for (const f of ['blockShapeDerivation.js', 'commandArgumentTypeDerivation.js', 'loginAckDerivation.js']) {
      const src = fs.readFileSync(path.join(__dirname, '..', 'src', 'client', f), 'utf8')
      assert.ok(!/META-INF\\\/\(\??:?jars\|jarjar\)/.test(src), `${f} must use the shared rule`)
      assert.ok(src.includes("require('./nestedJars')"), `${f} imports the shared rule`)
    }
  })
