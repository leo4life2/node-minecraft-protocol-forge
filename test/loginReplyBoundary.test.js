...
describe('unified login-reply boundary (HF36 ledger + HF38 window)', function () {
  ...
  it('P4 (HF36): unknown ids and post-close replies are refused with a receipt; the query verdicts stand; no ledger fails open', async () => {
    ...
    const s = loginWindow.loginWindowSummary(c)
    assert.strictEqual(s.counts.refused, 3)
    assert.strictEqual(s.counts.dropped, 0, 'a refused write is not a dropped query')
    assert.strictEqual(s.queries.find((q) => q.messageId === 6).outcome, 'unanswered-at-close')
  ...
  it('MED-4: after set_compression a deferred reply is HELD — login success arriving before the bound drops it (never a PLAY-state 0x02), a silent server gets it at the bound, an unarmed client after the two hops', async () => {
    ...
    // the window closed on success (the query is filed unanswered-at-close: the server completed without it) and the ledger refuses the late write first
    assert.ok(/already-closed-unanswered-at-close|state-is-play/.test(a.forgeDroppedLoginReplies[0].why), a.forgeDroppedLoginReplies[0].why)
    assert.strictEqual(loginWindow.loginWindowSummary(a).counts.unanswered, 1)
